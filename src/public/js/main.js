...
window.addEventListener('load', function() {
    ...
    const editor = new Editor({
        ...
        content: `${contentPost ? contentPost.innerHTML : ''}`,
        editorProps: {
            attributes: {
                class: 'format lg:format-lg h-96 bg-slate-200 px-2 overflow-x-auto dark:format-invert focus:outline-none format-blue max-w-none',
            },
        }
    });
    ...
    const formatFinalText = document.getElementById('getAllStyles')

    formatFinalText.addEventListener('click', (e) => {
        // Pega o HTML formatado do editor
        const content = editor.getHTML();
        
        // Atribui ao input hidden
        bodyInput.value = content.trim();
